...
async def build_lecture_plan_for_topic(
    ...
    system_prompt = (
        ...
        "- секции должны быть взаимно непересекающимися по содержанию;\n"
        "- не создавай обзорную секцию, которая подробно раскрывает темы будущих секций;\n"
        "- вводная секция должна только объяснять цель лекции и порядок изучения, без подробного разбора циклов, списков, строк, множеств, словарей, функций и файлов;\n"
        "- если тема выделена в отдельную секцию, не раскрывай её подробно в других секциях;\n"
        "- каждая ключевая тема должна встречаться как основной предмет объяснения только в одной секции;\n"
    ...
    plan_max_tokens = min(12000, max(4000, max_sections * 650))
    ...
    raw_text, _ = await proxy_completion(
        ...
        max_tokens=plan_max_tokens,  # даём модели договорить план до конца
    )
